...
//显示教师库中的教师信息
function showTeachers(ele, data, verify) {
    if (verify != 2)
        verify = 0;
    $(ele).empty();
    $.ajax({
            ...
            if (data.code == 100) {
                ...
                $(list).each(function (index, data) {
                    var TeacherInfo = '<li class="cf2"><div class="teacher_head"><a href="teacher_info.html?id=' + data.id + '" target="_blank" title="家教">'
                        ...
                        + '<span class="honor_label"><i class="' + (data.sex == 1 ? "female" : "male") + '"></i><span><i class="good" title="未签约"></i></span><span class="' + (data.isverify == 2 ? "on" : "") + '"><i class="v" title="' + (data.isverify == 2 ? "已审核证件" : "未审核证件") + '"></i></span></span> <em>' + data.currentStatus + '</em></div>'
...
function showConcat(item, type) {
    var result = "***";
    //学员预约
    if (item.status == 1) {
        if (type == 1) {
            result = item.teacherConcat;
        } else if (type == 2) {
            result = item.studentConcat;
        }
    }
    return result;
}